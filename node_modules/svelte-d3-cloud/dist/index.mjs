...
/* src/WordCloud.svelte generated by Svelte v3.38.3 */

function add_css() {
	var style = element("style");
	style.id = "svelte-ujvcz1-style";
	style.textContent = "div#wordcloud.svelte-ujvcz1{width:fit-content;height:fit-content}";
...
function instance($$self, $$props, $$invalidate) {
	...
	const layout = d3Cloud().size([width, height]).words(words).padding(padding).rotate(() => ~~(Math.random() * maxRotate) + minRotate).font(font).fontSize(d => Math.floor(d.count / maxWordCount * maxFontSize)).on("end", draw);

	function draw(words) {
		select("#wordcloud").append("svg").attr("width", layout.size()[0]).attr("height", layout.size()[1]).append("g").attr("transform", "translate(" + layout.size()[0] / 2 + "," + layout.size()[1] / 2 + ")").selectAll("text").data(words).enter().append("text").style("font-size", d => d.size + "px").style("font-family", font).style("fill", (_d, i) => fill(i)).attr("text-anchor", "middle").attr("transform", d => "translate(" + [d.x, d.y] + ")rotate(" + d.rotate + ")").text(d => d.text).on("click", onWordClick).on("mouseover", onWordMouserOver).on("mouseout", onWordMouseOut).on("mousemove", onWordMouseMove);
	}
	...
	$$self.$$set = $$props => {
		...
		if ("maxFontSize" in $$props) $$invalidate(5, maxFontSize = $$props.maxFontSize);
		if ("minRotate" in $$props) $$invalidate(6, minRotate = $$props.minRotate);
		if ("maxRotate" in $$props) $$invalidate(7, maxRotate = $$props.maxRotate);
		if ("scheme" in $$props) $$invalidate(8, scheme = $$props.scheme);
...
class WordCloud extends SvelteComponent {
	constructor(options) {
		super();
		if (!document.getElementById("svelte-ujvcz1-style")) add_css();
